...
export default function ProfilePage() {
  ...
  useEffect(() => {
    setOrders([
      { id: '1', date: '2024-01-15', total: 1299000, items: 2, status: 'Доставлен' },
      { id: '2', date: '2024-01-10', total: 249000, items: 1, status: 'В обработке' },
    ])
  }, [])
    ...
    <div className="frame-12">
      <div className="page-content">
        ...
        <div className="px-4 max-w-md mx-auto mt-4">
          <div className="bg-[#2A2A2A] p-6 rounded-2xl flex items-center gap-4">
            {user?.photo_url ? (
              <img
                src={user.photo_url}
                alt={user.first_name}
          ...
          <div className="grid grid-cols-2 gap-3 mt-4">
            <div className="bg-[#2A2A2A] p-4 text-center rounded-2xl">
          ...
          <div className="mt-6">
            <h3 className="font-semibold text-lg mb-3 text-white">📦 История заказов</h3>
            {orders.map((order) => (
              <div key={order.id} className="bg-[#2A2A2A] p-4 rounded-2xl mb-2">
                <div className="flex justify-between items-center">
                  <div>
                    <p className="font-semibold text-white">Заказ #{order.id}</p>
